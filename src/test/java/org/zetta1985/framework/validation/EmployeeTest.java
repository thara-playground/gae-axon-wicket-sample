...
public class EmployeeTest {
	...
	public static void init() {
		ResourceBundleMessageInterpolator delegate = new ResourceBundleMessageInterpolator(resource);
		
		Configuration<?> configure = Validation.byDefaultProvider().configure();
		((HibernateValidatorConfiguration)configure).messageInterpolator(new ParametarizedRessourceBundleMessageInterpolator(delegate));
	...
	public void validTest() {
		Employee emp = new Employee("1234", "AA");
		emp.text = "TEST";
		...
		Iterator<ConstraintViolation<Employee>> violations = results.iterator();
		assertEquals("nullであるべきです。", violations.next().getMessage());		
	}
