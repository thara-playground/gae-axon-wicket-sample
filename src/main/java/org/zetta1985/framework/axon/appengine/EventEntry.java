...
public class EventEntry {
	...
	static DomainEvent serialize(Entity entity, EventSerializer eventSerializer) {
		byte[] bytes = ((Text) entity.getProperty(SERIALIZED_EVENT)).getValue().getBytes(UTF8);
	 ...
	 */
	public static Query forAggregate(String type, String aggregateIdentifier, long firstSequenceNumber) {
	...
	public static Query forLastSnapshot(String type, String aggregateIdentifier) {
		return new Query(type)
		.addFilter(AGGREGATE_IDENTIFIER, Query.FilterOperator.EQUAL, aggregateIdentifier)
		.addSort(SEQUENCE_NUMBER, Query.SortDirection.ASCENDING);
	}
